...
def distance(vi,vj):
    ...
    ham su dung cong thuc tinh khoang cach giua 2 vector theo euclidean:
    Args:
        vi - vector vi
        vj - vector vj
        moi vector chua 3 chieu
    Returns:
    dis - khoang cach giua vi và vj
    """
    dis = np.sqrt((vi[0]-vj[0])**2 + (vi[1]-vj[1])**2 + (vi[2]-vj[2])**2)
    return dis

# thuc hien segmentation tren opencv source code tu opencv
def seg(img):
...
def kmeans_fast(features, k , num_iters = 100):
    """
        ...
        hieu qua hon
        su dung 1 so ham tu thu vien numpy giup kmeans chay nhanh hon: np.argmin 
        Args:
        ...
        Returns:
            assignment - 1 list chua thong tin ve vector thu i (voi i thuoc features) se thuoc nhom j (voi j thuoc k)
    """
    N, D = features.shape
...
def constrastStrecting(Input):
    ...
    for i in range (0,len(Output)):
        for j in range (0,len(Output[0])):
            Output[i][j]=(Input[i][j]-min_Input)*(((max_Output-min_Output)/(max_Input-min_Input))+min_Output)     
    return Output
...
def noiseRemove(img):
    """
        Khu nhieu cua 1 buc anh su dung ham morphologyEx tu thu vien opencv
        Args:
            img -  buc anh mau, la 1 ma tran voi kich thuoc (H,W,C)
        Return:
            noiseRemoveImg - anh da duoc khu nhieu
    """
    # noise removal
    kernel = np.ones((3,3),np.uint8)
    opening = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel, iterations = 1)
    noiseRemoveImg = cv2.dilate(opening,kernel,iterations = 1)
    return noiseRemoveImg
...
def imgModify(imgSrc, segments, k):
    """
        ...
        Args:
            ...
            k - so luong cluster 
    """
    # using imgRes to do many application to 
    #nen = cv2.imread("nen.jpg")
    #ghep anh
    ...
    #nen[segments!=0] = img[segments!=0]    
    #cv2.imwrite('result\\resul.jpg', nen)
...
if __name__ == "__main__":
    ...
    k = 2
    assignments = kmeans_fast(features, k, 7)
